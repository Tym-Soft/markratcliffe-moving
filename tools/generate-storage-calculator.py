#!/usr/bin/env python3
"""
Generate /resources/storage-calculator.html — an interactive volume +
weight calculator that lets a customer tick household items, see live
totals in cu ft / cu m / kg, and a lorry-size suggestion.

Item volumes are British Association of Removers (BAR) inventory-style
estimates that our crews use every week to plan Sussex jobs. They are
deliberately rounded — for an accurate quote the customer should
still book a free in-person survey.

Reuses the navbar + footer from another /resources/ page so chrome
...
# Category → list of (item name, cuft, cum, kg)
ITEMS: dict[str, list[tuple[str, float, float, float]]] = {
    'Living room': [
    ...
    'Bedroom': [
        ...
        ('Ottoman / blanket box',            10, 0.28, 20),
    ],
    'Kitchen / utility': [
    ...
    'Boxes & cartons': [
        ...
        ('Standard removals box',             3, 0.08, 12),
        ('Book box (1.5 cu ft)',              1.5, 0.04, 15),
        ('Linen box (large)',                 6, 0.17, 10),
        ('Wardrobe carton',                   6, 0.17, 15),
        ('Plastic crate',                     4, 0.11, 12),
...
def page_html() -> str:
    ...
    for cat, items in ITEMS.items():
        ...
        for name, cuft, cum, kg in items:
            ...
            rows.append(
                f'              <tr>\n'
                f'                <td><label for="item-{slug}">{name}</label></td>\n'
                f'                <td class="calc-vol"><span>{cuft} cu ft</span> &middot; <span>{cum} cu m</span> &middot; <span>{int(kg)} kg</span></td>\n'
                f'                <td class="calc-qty"><input id="item-{slug}" type="number" min="0" value="0" data-cuft="{cuft}" data-cum="{cum}" data-kg="{kg}" aria-label="Quantity of {name}"></td>\n'
                f'              </tr>'
            )
        rows_html = '\n'.join(rows)
        cat_blocks.append(
            f'        <details class="calc-cat"{" open" if cat == "Living room" else ""}>\n'
            f'          <summary>{cat} <span class="calc-count">({len(items)} items)</span></summary>\n'
            f'          <table class="calc-table">\n'
            f'            <tbody>\n'
            f'{rows_html}\n'
            f'            </tbody>\n'
            f'          </table>\n'
...
<head>
  ...
  <link href="../css/new-pages.css?v=20260561" rel="stylesheet">
...
{cats_html}
  ...
  <section class="np-section">
    <div class="np-inner">
      ...
      <p>Walk through your home room by room. For each item we include — there are {n_items} common household items in the list — type the quantity you own. The totals box at the top updates instantly. Three things to remember:</p>
      <ul>
  ...
  {footer}
  <script defer src="../js/storage-calculator.js?v=20260561"></script>
</body>
</html>
"""
